...
while { true } do {
	...
	_detected_vehicles = [ (getpos player) nearEntities [["Tank","APC","IFV","Car"], veh_action_distance] , { (count crew _x) == 0 && ((locked _x == 0 || locked _x == 1)) } ] call BIS_fnc_conditionalSelect;

	{
		...
		if ( !_next_vehicle_already_in_list ) then {
			_idact_next = _next_vehicle addAction [ "<t color='#FFFF00'>" + localize "STR_UNFLIP" + "</t> <img size='2' image='res\ui_flipveh.paa'/>", "scripts\client\actions\do_unflip.sqf", "", -950, true, true, "", "(_this distance _target < veh_action_distance) && (vehicle player == player)"];
